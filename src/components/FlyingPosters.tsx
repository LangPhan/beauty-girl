...
void main() {
  ...
  // When uPosition is 10 (center), make the plane completely flat
  // uPosition ranges from 5 to 15, so we remap to make 10 = 0 rotation
  float centerOffset = abs(uPosition - 10.0) / 5.0; // 0 at center, 1 at edges
  ...
  // Scale rotation by distance from center (0 at center, full rotation at edges)
  localprogress *= centerOffset;
...
class Media {
  ...
  onResize({
    ...
    this.padding = 5;
    this.height =
      this.plane.scale.y + this.padding;
    this.heightTotal =
      this.height * this.length;
    this.y =
      -this.heightTotal / 2 +
      (this.index + 0.5) * this.height;
  }
  ...
  update(scroll: ScrollState) {
    ...
    // Map so that center of screen (normalizedPos = 0) results in uPosition = 10
    // This makes the plane flat/visible when in the middle
    const position = map(
      normalizedPos,
      -1,
      1,
...
class Canvas {
  ...
  update() {
    // Add auto-scroll when user is not interacting
    if (
...
export default function FlyingPosters({
  ...
  return (
    <div
      ref={containerRef}
      className={`w-full h-full overflow-hidden absolute inset-0 z-2 pointer-events-none ${className}`}
      {...props}
    >
      <canvas
        ref={canvasRef}
        className="block w-full h-full z-2 pointer-events-auto"
      />
    </div>
  );
}
